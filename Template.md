...
$a * a ^ {φ(p) - 1} ≡ 1(mod \ p)$
由于$a * a^{-1} ≡ 1(mod \ p)$，在这里的$a^{-1}$不就是上面的$a ^ {φ(p) -1}$吗？，所以求出$a ^ {φ(p) - 1}$就好了。
...
再变一下，$a ^ {p - 2} ≡ a ^ {-1} (mod \ p)$ ，然后求出$a ^ {p - 2}$就可以了。
然后再看一下欧拉定理，如果p是质数，$φ(p) = p - 1$，那么我们求$a ^ {φ(p) - 1}$，也就是求$a ^ {p - 2}$。和费马小定理是一样的。

因此, 分数取余如下：
一般地, $p$为质数, 则直接乘$qmi(b, p - 2, p)$
